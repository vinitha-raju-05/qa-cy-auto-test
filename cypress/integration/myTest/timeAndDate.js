...
describe('Calender Sanity Test', () => {
    ...
    */
    ...
    //Verify there are no Duplicate Countries displayed in Dropdown
    it('TEST07: Duplicate Countries in dropdown', function () {
        var country = []
        var uniqueCountry = []
        cy.get('#sf_country option').each(($el, index) => {
            country.push($el.text())
            if (!uniqueCountry.includes($el.text())) {
                uniqueCountry.push($el.text())
            }
            expect(uniqueCountry[index]).to.equal(country[index])
        })
    ...
    //Verify when the Calendar header is displayed full, Show:long option should not be displayed 
    it('TEST13: Formatting - Display 1 Title either Long/compact', function () {

        cy.reload()
        cy.get('#ct1 h1').contains('Norway')
        cy.get('.ctxmenu').contains('Formatting').click()
        cy.get('ul.mn>li').eq(8)
            .contains('Title: Show long').should('not.be.visible').click()
    })
